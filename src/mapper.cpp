...
// init the static memo
    ...
    BaseMapper::BaseMapper(PathPositionHandleGraph *xidex,
            ...
            AlignerClient(estimate_gc_content(g)), xindex(xidex), gcsa(g), lcp(a),
            haplo_score_provider(haplo_score_provider), min_mem_length(1), mem_reseed_length(0), fast_reseed(true),
            fast_reseed_length_diff(0.45), adaptive_reseed_diff(true), adaptive_diff_exponent(0.065), hit_max(0),
            mapping_quality_method(Approx), max_mapping_quality(60), strip_bonuses(false), assume_acyclic(false),
...
// Use the GCSA2 index to find super-maximal exact matches.
    ...
    BaseMapper::find_mems_simple(string::const_iterator seq_begin,
        ...
        if (reseed_length) {
            ...
            std::sort(mems.begin(), mems.end(),
                      [](const MaximalExactMatch &m1, const MaximalExactMatch &m2) { return m1.begin < m2.begin; });
        }
        return mems;
    }
    ...
    vector <MaximalExactMatch> BaseMapper::find_fanout_mems(string::const_iterator seq_begin,
                                                            ...
                                                            vector <deque<pair < string::const_iterator, char>>
    ...
    - seq_begin);
    for (
    auto seq_it = seq_begin, qual_it = qual_begin;
    seq_it !=
    seq_end;
    ++seq_it, ++qual_it) {
    qual_pos.
    emplace_back(*(qual_it), seq_it
    );
}
make_heap(qual_pos
.
...
end(), greater<decltype(qual_pos)::value_type>()
...
while (start_cursor >=
seq_begin &&*start_cursor
== 'N' && !do_fanout[start_cursor - seq_begin]) {
...
'A', 'C', 'G', 'T'}) {
stack.
emplace_back(full_range, seq_begin, seq_end, start_cursor, ch
);
}
}
else {
...
stack.
emplace_back(full_range, seq_begin, seq_end, start_cursor, *start_cursor
);
}
}
}

vector <tuple<gcsa::range_type, string::const_iterator, string::const_iterator,
        deque < pair < string::const_iterator, char>>>>
search_results;
...
if (!
use_fanout_char &&do_fanout[cursor - seq_begin]
) {
#ifdef debug_mapper
...
'A', 'C', 'G', 'T'}) {
stack.
emplace_back(range, seq_begin, match_end, cursor, nt, prev_iter_jumped_lcp, fanout_breaks
);
}
...
if (
gcsa::Range::empty(range)
|| match_end - cursor > gcsa->
...
match_end - cursor - 1 <= fanout_length_threshold) {
...
search_results.
emplace_back(prev_range, cursor
+ 1, match_end, fanout_breaks);
...
match_end - match_begin <= fanout_length_threshold) {
// this fan-out search didn't find a long enough match for us to think
// that these are non-random matches
#ifdef debug_mapper
...
#endif
search_results.
emplace_back(prev_range, match_begin, match_end, fanout_breaks
);
}
...
.
first< get<1>(search_results.back())
) {
get<3>(search_results
.
...
#endif
search_results.
emplace_back(range, seq_begin, match_end, fanout_breaks
);
}
}
...
[](const tuple <gcsa::range_type, string::const_iterator, string::const_iterator, deque<
        pair < string::const_iterator, char>>>& a,
const tuple <gcsa::range_type, string::const_iterator, string::const_iterator, deque<
        pair < string::const_iterator, char>>>& b) {
return (
get<1>(a)
<
get<1>(b)
...
get<2>(b)
&&
gcsa::Range::length(get<0>(a)
) >
gcsa::Range::length(get<0>(b)
)));
});
...
// this one
while (
get<2>(search_results[i])
<
get<1>(search_results[j])
) {
++
i;
}
...
search_results[j - res_removed_so_far] =
move(search_results[j]);
}
}
...
for (
auto it = search_results.begin(), end = search_results.end();
it !=
end;
++it) {
...
mems.
emplace_back(get<1>(search_result), get<2>(search_result), get<0>(search_result),
        gcsa
->
count(get<0>(search_result)
));
if (!hard_hit_max || mems.

back()

.match_count<hard_hit_max) {
if (hit_max) {
gcsa->
locate(mems
.
...
.
queried_count = mems.back().nodes.size();
...
#endif
mem_fanout_breaks->
emplace_back(move(get < 3 > (search_result))
);
}
else if (!
get<3>(search_result)
...
.nodes) {
paths.
emplace_back(walk_fanout_path(get < 1 > (search_result),
                              get < 2 > (search_result),
                              get < 3 > (search_result),
                              pos)
);
...
.
end = fanout_break.first;
if (fanout_break.first + 1 == seq_end) {
break;
}
mems.
emplace_back(fanout_break
.first + 1,
get<2>(search_result),
        mems
.
...
.
queried_count = mems[mems.size() - 2].queried_count;
...
.nodes.
emplace_back(gcsa::Node::encode(id(path_step),
...
(mem_fanout_breaks ? mem_fanout_breaks->
...
empty() :
        find(mems.front().begin, mems.front().end, 'N')
...
vector <pair<MaximalExactMatch, vector < size_t>>>
sub_mems;
...
.begin, min_sub_mem_length, sub_mems);
...
for (
auto &sub_mem_and_parents
: sub_mems) {
mems.
emplace_back(move(sub_mem_and_parents.first)
);
if (mem_fanout_breaks) {
mem_fanout_breaks->
...
containment_graph.emplace_back(0,
move(sub_mem_and_parents
.second));
}
...
mems[i - num_removed_so_far] =
move(mems[i]);
if (mem_fanout_breaks) {
(*mem_fanout_breaks)[i - num_removed_so_far] =
move((*mem_fanout_breaks)[i]);
}
}
}
...
vector <MaximalExactMatch> BaseMapper::find_mems_deep(string::const_iterator seq_begin,
    ...
    if (reseed_length) {
        ...
        for (size_t i = 0; i < mems.size(); i++) {
            if (use_diff_based_fast_reseed && adaptive_reseed_diff) {
                sub_mem_containment_graph[i].first = max<int>(get_adaptive_min_reseed_length(mems[i].length()),
                                                              min_mem_length);
            } else if (use_diff_based_fast_reseed) {
                sub_mem_containment_graph[i].first = max<int>(ceil(fast_reseed_length_diff * mems[i].length()),
                                                              min_mem_length);
            } else {
        ...
        for (int depth = 0; depth < max_sub_mem_recursion_depth; depth++) {
            ...
            for (int i = layer_begin; i < layer_end; i++) {
                ...
                if (mem.length() >= min_mem_length &&
                    ...
                    string::const_iterator possible_containment_boundary =
                            i - 1 >= layer_begin ? mems[i - 1].begin : mems[i].end;
    ...
    std::for_each(mems.begin(), mems.end(),
                  [&total_mems](const MaximalExactMatch &mem) { total_mems += mem.nodes.size(); });
...
sub_mems_out.
emplace_back(MaximalExactMatch(sub_mem_begin, sub_mem_end, last_range),
...
if (sub_mem_end >
next_mem_end &&sub_mem_end
- mem.begin >= min_mem_length && !prev_iter_jumped_lcp) {
sub_mems_out.
emplace_back(MaximalExactMatch(mem.begin, sub_mem_end, range),
        vector<size_t>(1, mem_idx)
);
#ifdef debug_mapper
                                                                                                                        cerr << "adding sub-MEM ";
...
// annotate the MEMs
for (
pair <MaximalExactMatch, vector<size_t>> &sub_mem_and_parents
: sub_mems_out) {
// count in entire range, including parents
sub_mem_and_parents.first.
match_count = gcsa->count(sub_mem_and_parents.first.range);
...
// how many times does the parent MEM occur in the index?
size_t parent_range_count = use_approx_sub_mem_count ? gcsa::Range::length(mems[mem_idx].range)
                                                     : mems[mem_idx].match_count;
...
(relative_idx >=
sub_mem_thinning_burn_in &&(relative_idx
- sub_mem_thinning_burn_in) % sub_mem_count_thinning == 0)) {

if ((use_approx_sub_mem_count ?
gcsa::Range::length(range)
: gcsa->
count(range)
) <= parent_range_count) {
...
bool possibly_redundant = (right_search_bound == mems[mem_idx].end &&
                           probe_string_begin >= leftmost_guaranteed_disjoint_bound);
...
// record the sub-MEM
sub_mems_out.
emplace_back(MaximalExactMatch(probe_string_begin, right_search_bound, sub_mem_range),
        vector<size_t>(1, mem_idx)
);
...
vector <MaximalExactMatch> BaseMapper::find_stripped_matches(string::const_iterator seq_begin,
    ...
    if (strip_length <= 0) {
        throw runtime_error(
                "error:[vg::Mapper] strip match length must be positive, set to " + to_string(strip_length));
    }
    if (target_count <= 0) {
        throw runtime_error(
                "error:[vg::Mapper] target match count must be positive, set to " + to_string(target_count));
    }
...
void BaseMapper::prefilter_redundant_sub_mems(vector < MaximalExactMatch > &mems,
                                              vector < pair < int, vector < size_t
>>>& sub_mem_containment_graph) {
...
for (
int64_t i = mems.size() - 1;
i >= 0; i--) {

if (sub_mem_containment_graph[i].second.
...
for (
size_t j
: sub_mem_containment_graph[i].second) {
#ifdef debug_mapper
...
extended_hit_positions.
emplace(id(hit_pos), is_rev(hit_pos), offset(hit_pos)
+ relative_offset);
#ifdef debug_mapper
...
k++) {
if (extended_hit_positions.
count(make_pos_t(nodes[k])
)) {
...
void BaseMapper::precollapse_order_length_runs(string::const_iterator seq_begin,
    ...
    collapsable_pairs(order_length_mems.size());
...
if (min_hit_count <=
max_rescue_hit_count &&has_order_length_mem
) {
...
if (hit_max) {
gcsa->
locate(mems[min_hit_mem]
.range, hit_max, mems[min_hit_mem].nodes);
} else {
gcsa->
locate(mems[min_hit_mem]
.range, mems[min_hit_mem].nodes);
}
}
}
}

size_t BaseMapper::get_adaptive_min_reseed_length(size_t
parent_mem_length) {
// extend memo until it contains this parent MEM length
while (adaptive_reseed_length_memo.
...
void BaseMapper::apply_haplotype_consistency_scores(const vector<Alignment *> &alns) {
    ...
    for (auto *aln : alns) {
        ...
        if (std::isnan(haplotype_logprob) && path_valid) {
            ...
            cerr
                    << "warning:[vg::Mapper]: NAN population score obtained for read with ostensibly successful query. Changing to failure."
                    << endl;
            path_valid = false;
        }
...
void PairedEndMapper::set_fragment_length_distr_params(size_t
maximum_sample_size,
...
cerr << "warning:[vg::PairedEndMapper] overwriting a fragment length distribution that has already been estimated" <<
endl;
}
...
Mapper::Mapper(PathPositionHandleGraph *xidex,
               ...
               gcsa::LCPArray *a,
               haplo::ScoreProvider *haplo_score_provider) :
        BaseMapper(xidex, g, a, haplo_score_provider), thread_extension(10), max_multimaps(1), min_multimaps(4),
        max_attempts(0), min_cluster_length(0), softclip_threshold(0), max_softclip_iterations(10), min_identity(0),
        max_target_factor(128), extra_multimaps(512), band_multimaps(4), always_rescue(false),
        max_cluster_mapping_quality(1024), use_cluster_mq(false), simultaneous_pair_alignment(true), drop_chain(0.2),
        mq_overlap(0.2), mate_rescues(0), maybe_mq_threshold(0), min_banded_mq(0), max_band_jump(0),
        patch_alignments(false), identity_weight(2), pair_rescue_hang_threshold(0.7), pair_rescue_retry_threshold(0.5),
...
Alignment Mapper::align_to_graph(const Alignment &aln,
    ...
    vector<MaximalExactMatch> mems;
    return align_to_graph(aln, graph, mems, do_flip, traceback, pinned_alignment, pin_left, banded_global, false,
                          keep_bonuses);
}
...
Alignment Mapper::align_to_graph(const Alignment &aln,
    ...
    if (banded_global) {
        ...
        size_t
        band_padding = permissive_banding ? max(max_span, (size_t) 1) : band_padding_override;
...
Alignment
Mapper::align(const string &seq, int kmer_size, int stride, int max_mem_length, int band_width, int band_overlap,
...
vector <pos_t> Mapper::likely_mate_positions(const Alignment &aln, bool is_first_mate) {
    ...
    for (auto &mapping : aln.path().mapping()) {
        auto pos_offs = algorithms::nearest_offsets_in_paths(xindex, make_pos_t(mapping.position()),
                                                             aln.sequence().size());
        for (auto &p : pos_offs) {
    ...
    for (auto &mate_pos : mate_positions) {
        ...
        int get_at_least = (!frag_stats.cached_fragment_length_mean ? frag_stats.fragment_max
                                                                    : min(frag_stats.fragment_max / 2,
                                                                          (int64_t) max(
                                                                                  (double) frag_stats.cached_fragment_length_stdev *
                                                                                  10.0,
        ...
        algorithms::extract_context(*xindex, graph, xindex->get_handle(id(mate_pos), is_rev(mate_pos)),
                                    offset(mate_pos), get_at_least);
...
#endif
            if (aln2.score() > max_mate2_score && (double) aln2.score() / perfect_score > min_threshold &&
                pair_consistent(mate1, aln2, accept_pval)) {
...
#endif
            if (aln1.score() > max_mate1_score && (double) aln1.score() / perfect_score > min_threshold &&
                pair_consistent(aln1, mate2, accept_pval)) {
                if (!traceback) { // now get the traceback
        ...
        auto pos_consistent =
                [&](const pair<size_t, bool> &p1, const pair<size_t, bool> &p2) {
                    ...
                    if (frag_stats.fragment_size) {
                        bool orientation_ok =
                                (frag_stats.cached_fragment_orientation_same && fwd1 == fwd2) || fwd1 != fwd2;
                        bool direction_ok =
                                frag_stats.cached_fragment_direction && (!fwd1 && len >= 0 || fwd1 && len <= 0)
                                || (fwd1 && len >= 0 || !fwd1 && len <= 0);
                        bool length_ok = frag_stats.fragment_length_pval(abs(len)) >
                                         pval;//|| pval == 0 && abs(len) < frag_stats.fragment_size;
...
pair <vector<Alignment>, vector<Alignment>> Mapper::align_paired_multi(
    ...
    for (auto &mem : mems1)
        if (mem.primary && mem.match_count)
            mem_max_length1 = max(mem_max_length1, (int) mem.length());
    double maybe_mq1 = 0;
    if (mems1.size()) {
    ...
    for (auto &mem : mems2)
        if (mem.primary && mem.match_count)
            mem_max_length2 = max(mem_max_length2, (int) mem.length());
    double maybe_mq2 = 0;
    if (mems2.size()) {
    ...
    */
    ...
    if (debug)
        cerr << "maybe_mq1 " << read1.name() << " " << maybe_mq1 << " " << total_multimaps << " " << mem_max_length1
             << " " << longest_lcp1 << " " << total_multimaps << " " << mem_read_ratio1 << " " << fraction_filtered1
             << " " << max_possible_mq << " " << total_multimaps << endl;
    if (debug)
        cerr << "maybe_mq2 " << read2.name() << " " << maybe_mq2 << " " << total_multimaps << " " << mem_max_length2
             << " " << longest_lcp2 << " " << total_multimaps << " " << mem_read_ratio2 << " " << fraction_filtered2
             << " " << max_possible_mq << " " << total_multimaps << endl;
    ...
    auto transition_weight = [&](const MaximalExactMatch &m1, const MaximalExactMatch &m2) {
        ...
        if (m1.fragment < m2.fragment) {
            ...
            int64_t approx_dist = (!frag_stats.fragment_size ? min(d.first, d.second)
                                                             : (frag_stats.cached_fragment_orientation_same ? d.first
                                                                                                            : d.second));
    ...
    auto show_paired_clusters = [&](void) {
        ...
        for (auto &cluster_ptr : cluster_ptrs) {
            ...
            cerr << cluster1.size() << " " << cluster2.size() << " MEMs covering " << cluster_coverage(cluster1) << " "
                 << cluster_coverage(cluster2) << " @ ";
            cerr << " cluster1: ";
    ...
    auto sort_and_dedup = [&](void) {
        ...
        for (auto &p : aln_ptrs) {
            ...
            frag_stats.save_frag_lens_to_alns(aln1, aln2, approx_frag_lengths, xindex,
                                              pair_consistent(aln1, aln2, 1e-6));
        }
    ...
    for (auto &p1 : aln_ptrs) {
        ...
        for (auto &p2 : aln_ptrs) {
            ...
            bool consistent1a2b = aln1a.score() > mix_threshold && aln2b.score() > mix_threshold &&
                                  pair_consistent(aln1a, aln2b, 1e-6);
            bool consistent1b2a = aln1b.score() > mix_threshold && aln2a.score() > mix_threshold &&
                                  pair_consistent(aln1b, aln2a, 1e-6);
            if (consistent1a2b) {
    ...
    bool max_first = results.first.size() && (read1_max_score == results.first.front().score() &&
                                              read2_max_score == results.second.front().score());
...
Alignment
Mapper::align_maybe_flip(const Alignment &base, HandleGraph &graph, bool flip, bool traceback, bool banded_global,
                         bool xdrop_alignment) {
...
Alignment
Mapper::align_maybe_flip(const Alignment &base, HandleGraph &graph, const vector <MaximalExactMatch> &mems, bool flip,
                         bool traceback, bool banded_global, bool xdrop_alignment) {
    Alignment aln = base;
...
void
FragmentLengthStatistics::record_fragment_configuration(const Alignment &aln1, const Alignment &aln2, Mapper *mapper) {
...
vector <Alignment>
Mapper::make_bands(const Alignment &read, int band_width, int band_overlap, vector <pair<int, int>> &to_strip) {
...
vector <Alignment>
Mapper::align_banded(const Alignment &read, int kmer_size, int stride, int max_mem_length, int band_width,
                     int band_overlap, bool xdrop_alignment) {
    ...
    auto do_band = [&](int i) {
        ...
        double cluster_mq = 0;
        malns = align_multi_internal(true, bands[i], kmer_size, stride, max_mem_length, bands[i].sequence().size(), 0,
                                     cluster_mq, band_multimaps, extra_multimaps, nullptr, xdrop_alignment);
...
#pragma omp parallel for
    ...
    // cost function
    auto
    transition_weight = [&](const Alignment &aln1, const Alignment &aln2,
    ...
    if (patch_alignments) {
        for (auto &aln : alignments) {
            ...
            aln = patch_alignment(aln, band_width / 2, true,
                                  false); // can't use xdrop_alignment here as it needs seeds to work correctly
        }
    }
    // sort the alignments by score
    std::sort(alignments.begin(), alignments.end(),
              [](const Alignment &aln1, const Alignment &aln2) { return aln1.score() > aln2.score(); });
    if (alignments.empty()) {
...
void Mapper::compute_mapping_qualities(vector < Alignment > &alns, double
cluster_mq,
double mq_estimate,
double mq_cap
) {
...
case Approx:
aligner->
compute_mapping_quality(alns, max_mq,
true, cluster_mq, use_cluster_mq, sub_overlaps, mq_estimate, maybe_mq_threshold, identity_weight);
break;
case Exact:
aligner->
compute_mapping_quality(alns, max_mq,
false, cluster_mq, use_cluster_mq, sub_overlaps, mq_estimate, maybe_mq_threshold, identity_weight);
break;
default: // None
break;
}
...
void Mapper::compute_mapping_qualities(pair <vector<Alignment>, vector<Alignment>> &pair_alns, double cluster_mq,
                                       double mq_estimate1, double mq_estimate2, double mq_cap1, double mq_cap2) {
    ...
    switch (mapping_quality_method) {
        case Approx:
            aligner->compute_paired_mapping_quality(pair_alns, frag_weights, max_mq1, max_mq2, true, cluster_mq,
                                                    use_cluster_mq, sub_overlaps1, sub_overlaps2, mq_estimate1,
                                                    mq_estimate2, maybe_mq_threshold, identity_weight);
            break;
        case Exact:
            aligner->compute_paired_mapping_quality(pair_alns, frag_weights, max_mq1, max_mq2, false, cluster_mq,
                                                    use_cluster_mq, sub_overlaps1, sub_overlaps2, mq_estimate1,
                                                    mq_estimate2, maybe_mq_threshold, identity_weight);
            break;
        default: // None
...
vector <Alignment> Mapper::score_sort_and_deduplicate_alignments(vector < Alignment > &all_alns,
...
for(
auto it = alignment_by_score.rbegin();
it != alignment_by_score.
...
deterministic_shuffle(sorted_unique_alignments
.

begin(), sorted_unique_alignments
...
void Mapper::filter_and_process_multimaps(vector < Alignment > &sorted_unique_alignments, int
total_multimaps) {
if (sorted_unique_alignments.

size()

> total_multimaps){
sorted_unique_alignments.
resize(total_multimaps);
}

// TODO log best alignment score?
for(
size_t i = 0;
i<sorted_unique_alignments.

size();

i++) {
// Mark all but the first, best alignment as secondary
sorted_unique_alignments[i].
set_is_secondary(i
> 0);
}
}
...
vector <Alignment>
Mapper::align_multi(const Alignment &aln, int kmer_size, int stride, int max_mem_length, int band_width,
                    int band_overlap, bool xdrop_alignment, bool trekmer, bool printMin) {
    double cluster_mq = 0;
    Alignment clean_aln;
    ...
    return align_multi_internal(true, clean_aln, kmer_size, stride, max_mem_length, band_width, band_overlap,
                                cluster_mq, max_multimaps, extra_multimaps, nullptr, xdrop_alignment, trekmer,
                                printMin);
}
...
#pragma omp critical
    ...
    if (aln.sequence().size() > band_width) {
        ...
        return vector<Alignment>{
                align_banded(aln, kmer_size, stride, max_mem_length, band_width, band_overlap, xdrop_alignment)};
    }
    ...
    if (restricted_mems != nullptr) {
        ...
        alignments = align_mem_multi(aln, *restricted_mems, cluster_mq, longest_lcp, fraction_filtered, max_mem_length,
                                     keep_multimaps, additional_multimaps_for_quality, xdrop_alignment);
    } else {
        ...
        if (trekmer == false) {
            ...
            // query mem hits
            alignments = align_mem_multi(aln, mems, cluster_mq, longest_lcp, fraction_filtered, max_mem_length,
                                         keep_multimaps, additional_multimaps_for_quality, xdrop_alignment);
            ...
            if (!mems.empty()) {
                ...
                bool found = false;
                int initialMatch = mems[0].nodes.size();
                bool altPath = false;
                string alignToStore;
                bool toPrint = false;

                for (auto id_1 : mems[0].nodes) {

                    auto pos_node1 = make_pos_t(gcsa::Node::id(id_1), gcsa::Node::rc(id_1),
                                                gcsa::Node::offset(id_1));  //from gcsa to xg node

                    auto node1 = gcsa::Node::id(id_1);
                    bool rc_node1 = gcsa::Node::rc(id_1);  // (true = is reverse)
                    auto offset_node1 = gcsa::Node::offset(id_1);
                    string seq_node1 = xindex->get_sequence(xindex->get_handle(node1, rc_node1));

                    vector<pair < int, string> > otherNodes; //to store all nodes

                    int dimNode = seq_node1.size();
                    int nOnNode1 = dimNode - offset_node1; //Number of nucleotides on node 1

                    std::string readToSearch = aln.sequence();
                    std::string strToSearch = readToSearch.substr(
                            nOnNode1);
                    if (!strToSearch.empty() && strToSearch[strToSearch.size() - 1] == '\r')
                        strToSearch.erase(strToSearch.size() - 1);

                    /* Use this to get min e max node of graph.xg
                int last_node = xindex->max_node_id();
                int first_node = xindex->min_node_id();

                cout << last_node << endl;
                cout << first_node << endl; */

                    int tmp_node = (int) node1;

                    bool noMatch = false;

                    int stop_condition = (strToSearch.size() + 1) * 2;
                    //cout << stop_condition << endl;

                    int lastNode = node1;
                    int edgeOnNode = 0;

                    while (strToSearch.size() != 0 || stop_condition == 0) {

                        if (rc_node1 == true) {  //Searching for a node with an ID lower then the previous ones
                            ...
                            if (tmp_node < 1 || tmp_node > xindex->max_node_id()) {
                                ...
                                pair<int, string> tmp;
                                tmp.first = tmp_node;
                                tmp.second = xindex->get_sequence(xindex->get_handle(tmp_node, rc_node1));
                                if (strToSearch.size() < tmp.second.size()) {
                                    int posStr = tmp.second.rfind(strToSearch, 0);
                                    if (posStr != std::string::npos && posStr == 0) {
                                        //cout << tmp_node << "OK" << endl;
                                        if (xindex->has_edge(xindex->get_handle(lastNode, rc_node1),
                                                             xindex->get_handle(tmp_node, rc_node1))) {

                                        strToSearch = strToSearch.substr(posStr + strToSearch.size());
                                        lastNode = tmp_node;
                                        edgeOnNode = 0;
                                        otherNodes.push_back(tmp);
                                        } else {
                                            edgeOnNode++;
                                            if (edgeOnNode == 4) {
                                                stop_condition = 0;
                                    ...
                                    int posStr = strToSearch.rfind(tmp.second, 0);
                                    if (posStr != std::string::npos && posStr == 0) {
                                        //cout << tmp_node << "OK" << endl;
                                        if (xindex->has_edge(xindex->get_handle(lastNode, rc_node1),
                                                             xindex->get_handle(tmp_node, rc_node1))) {

                                        strToSearch = strToSearch.substr(posStr + tmp.second.size());
                                        lastNode = tmp_node;
                                        edgeOnNode = 0;
                                        otherNodes.push_back(tmp);
                                    } else {
                        ...
                        } else {  //Searching for a node greater then the previous ones
                            ...
                            stop_condition--;
                            if (stop_condition == 0) noMatch = true;
                            //cout << stop_condition << endl;
                    ...
                    if (noMatch == false) {
                        ...
                        //Searching for paths and offset for node 1
                        vector<pair < int, string> > nodesPath;
                        vector<pair < int, string> > nodesOffset;

                        auto pos_offs_1 = algorithms::nearest_offsets_in_paths(
                                dynamic_cast<PathPositionHandleGraph *>(&*xindex), pos_node1, 1000);  //for node1!

                        bool print;
                        if (printMin == true) print = false;
                        if (printMin == false) print = true;

                        for (auto &p : pos_offs_1) {

                            path_handle_t path_handle = p.first;
                            pair<int, string> tmp;
                            tmp.first = node1;
                            tmp.second = xindex->get_path_name(path_handle);

                            if (tmp.second.rfind("_alt", 0) == 0) {
                                print = true;  // starts with prefix alt path
                                altPath = true;
                            }

                            nodesPath.push_back(tmp);

                            for (pair<size_t, bool> &y : p.second) {

                                pair<int, string> tmp;
                                tmp.first = node1;
                                tmp.second = std::to_string(y.first);
                                nodesOffset.push_back(tmp);

                            }
                        }

                        //Searching for paths and offset for all nodes

                        if (!otherNodes.empty()) {

                            for (pair<int, string> nodeTmp : otherNodes) {

                                auto pos_node = make_pos_t(nodeTmp.first, rc_node1, 0);
                                auto pos_offs = algorithms::nearest_offsets_in_paths(
                                        dynamic_cast<PathPositionHandleGraph *>(&*xindex), pos_node, 1000);

                                for (auto &p : pos_offs) {

                                    path_handle_t path_handle = p.first;
                                    pair<int, string> tmp;
                                    tmp.first = nodeTmp.first;
                                    tmp.second = xindex->get_path_name(path_handle);
                        ...
                        if (print == true) { //print align

                            std::string outputToPrint;

                            outputToPrint.append("{\"identity\": 1.0, \"mapping_quality\": 60,");
                            //cout << "{\"identity\": 1.0, " ;
                            std::string sequenceRead = "\"" + aln.name() + "\"";

                            outputToPrint.append(" \"name\": " + sequenceRead + ",");
                            //cout << " \"name\": " <<  sequenceRead << ",";

                            outputToPrint.append("  \"path\": { \"mapping\": [ ");
                            //cout << "  \"path\": { \"mapping\": [ ";

                            //first node of match
                            int lOnnode;
                            if (rc_node1 == true) {
                                lOnnode = dimNode - offset_node1;

                                outputToPrint.append("{\"edit\": [{\"from_length\": " + std::to_string(lOnnode) +
                                              ", \"to_length\": " + std::to_string(lOnnode) +
                                              "}], \"position\": {\"is_reverse\": true, \"node_id\": \"" +
                                              std::to_string(node1) + "\", \"offset\": \"" +
                                              std::to_string(offset_node1) + "\"}, \"rank\": \"1\"}, ");
                                //cout << "{\"edit\": [{\"from_length\": " << lOnnode << ", \"to_length\": " << lOnnode << "}], \"position\": {\"is_reverse\": true, \"node_id\": \"" << node1 << "\", \"offset\": \"" << offset_node1 << "\"}, \"rank\": \"1\"},";
                            } else {
                                lOnnode = dimNode - offset_node1;
                                outputToPrint.append("{\"edit\": [{\"from_length\": " + std::to_string(lOnnode) +
                                              ", \"to_length\": " + std::to_string(lOnnode) +
                                              "}], \"position\": {\"node_id\": \"" + std::to_string(node1) +
                                              "\", \"offset\": \"" + std::to_string(offset_node1) +
                                              "\"}, \"rank\": \"1\"}, ");
                                //cout << "{\"edit\": [{\"from_length\": " << lOnnode << ", \"to_length\": " << lOnnode << "}], \"position\": {\"node_id\": \"" << node1 << "\", \"offset\": \"" << offset_node1 << "\"}, \"rank\": \"1\"},";
                            }

                            int nucleotidiMatch = lOnnode;
                            int charOnLastNode = 0;

                            //other nodes of match
                            if (!otherNodes.empty()) {

                                int numOfnode = 1;
                                int lastNode = otherNodes.size();
                                    ...
                                    if (rc_node1 == true) {
                                        ...
                                        if (numOfnode - 1 == lastNode) {
                                            ...
                                            outputToPrint.append(
                                                    ...
                                                    ", \"to_length\": " + std::to_string(charOnnode) +
                                                    "}], \"position\": {\"is_reverse\": true, \"node_id\": \"" +
                                                    std::to_string(nodeTmp.first) + "\"}, \"rank\": \"" +
                                                    std::to_string(numOfnode) + "\"}");
                                            //cout << "{\"edit\": [{\"from_length\": " << charOnnode << ", \"to_length\": " << charOnnode << "}], \"position\": {\"is_reverse\": true, \"node_id\": \"" << nodeTmp.first << "\"}, \"rank\": \"" << numOfnode << "\"}";

                                        } else {
                                            charOnnode = nodeTmp.second.size();
                                            outputToPrint.append(
                                                    "{\"edit\": [{\"from_length\": " + std::to_string(charOnnode) +
                                                    ", \"to_length\": " + std::to_string(charOnnode) +
                                                    "}], \"position\": {\"is_reverse\": true, \"node_id\": \"" +
                                        ...
                                        nucleotidiMatch = nucleotidiMatch + charOnnode;
                                        ...
                                        if (numOfnode - 1 == lastNode) {
                                            ...
                                            outputToPrint.append(
                                                    ...
                                                    std::to_string(nodeTmp.first) + "\"}, \"rank\": \"" +
                                                    std::to_string(numOfnode) + "\"}");
                                            //cout << "{\"edit\": [{\"from_length\": " << charOnnode << ", \"to_length\": " << charOnnode << "}], \"position\": {\"node_id\": \"" << nodeTmp.first << "\"}, \"rank\": \"" << numOfnode << "\"}";
                            ...
                            // string_quality_short_to_char(aln.quality())
                            //std::string base64_decode(std::string const& s, bool remove_linebreaks = false);
                            outputToPrint.append("\"quality\": \"" + base64_encode(aln.quality()) + "\", ");
                            //cout << "\"quality\": \"" << base64_encode(aln.quality()) << "\",";  //forse .quality()

                            outputToPrint.append("\"refpos\": [");
                            //cout << "\"refpos\": [" ;
                            if (!nodesPath.empty()) {

                                std::vector<string> mypath;
                                for (pair<int, string> nodeTmp : nodesPath) {
                                    mypath.push_back(nodeTmp.second);
                                }

                                // using default comparison:
                                std::vector<string>::iterator it;
                                it = std::unique(mypath.begin(), mypath.end());

                                mypath.resize(std::distance(mypath.begin(), it));

                                int numPath = mypath.size();
                                int pathWrite = 0;

                                for (string nodeTmp : mypath) {

                                    pathWrite++;

                                    if (nodeTmp.rfind("_alt", 0) == 0) {

                                        if (rc_node1 == true) {

                                            if (pathWrite == numPath) {
                                                outputToPrint.append("{ \"is_reverse\": true, \"name\": \"" + nodeTmp + "\"}");
                                                //cout <<  "{ \"is_reverse\": true, \"name\": \"" << nodeTmp << "\", \"offset\": \"" << offsetToWrite << "\"}";
                                            } else {
                                                outputToPrint.append(
                                                        "{ \"is_reverse\": true, \"name\": \"" + nodeTmp + "\"},");
                                                //cout <<  "{ \"is_reverse\": true, \"name\": \"" << nodeTmp << "\", \"offset\": \"" << offsetToWrite << "\"},";
                                            }

                                        } else {
                                            if (pathWrite == numPath) {
                                                outputToPrint.append("{ \"name\": \"" + nodeTmp + "\"}");
                                                //cout <<  "{ \"name\": \"" << nodeTmp << "\"}";
                                            } else {
                                                outputToPrint.append("{ \"name\": \"" + nodeTmp + "\"},");
                                                //cout <<  "{ \"name\": \"" << nodeTmp << "\"},";
                                        ...
                                        if (!nodesOffset.empty()) {
                                            if (rc_node1 == true) {
                                                int n = nodesOffset.size() - 1;
                                                int tmpOff = std::stoi(nodesOffset[n].second) - charOnLastNode -
                                                             1;
                                                offsetToWrite = std::to_string(tmpOff);
                                            } else {
                                                offsetToWrite = nodesOffset[0].second;
                                            }
                                        }
                                        ...
                                        if (rc_node1 == true) {
                                            ...
                                            if (pathWrite == numPath) {
                                                outputToPrint.append("{ \"name\": \"" + nodeTmp + "\", \"offset\": \"" +
                                                              offsetToWrite + "\"}");
                                                //cout <<  "{ \"name\": \"" << nodeTmp << "\", \"offset\": \"" << offsetToWrite << "\"}";
                                            } else {
                                                outputToPrint.append("{ \"name\": \"" + nodeTmp + "\", \"offset\": \"" +
                                                              offsetToWrite + "\"},");
                                                //cout <<  "{ \"name\": \"" << nodeTmp << "\", \"offset\": \"" << offsetToWrite << "\"},";
                            ...
                            outputToPrint.append("], \"score\": 111,");
                            //cout << "], \"score\": 111,";

                            std::string readToSearch = aln.sequence();
                            if (!readToSearch.empty() && readToSearch[readToSearch.size() - 1] == '\r')
                                readToSearch.erase(readToSearch.size() - 1);

                            outputToPrint.append(" \"sequence\": \"" + readToSearch + "\", \"time_used\": 2000.0");
                            //cout << " \"sequence\": \"" << readToSearch << "\", \"time_used\": 2000.0";
                            ...
                            if ( initialMatch >= 1 && altPath == false) {

                                alignToStore = outputToPrint;
                                toPrint = true;
                            } else {
                                cout << outputToPrint << endl;
                            }
                        ...
                        if(toPrint == true) cout << alignToStore << endl;
                    }
...
Alignment
Mapper::align(const Alignment &aln, int kmer_size, int stride, int max_mem_length, int band_width, int band_overlap,
              bool xdrop_alignment) {
                ...
                for (int k = 0; k < bands.size(); ++k) {
                    ...
                    if (i == 0 && j == 0 && k == 0) {
                        ...
                        for (auto &pos : band_ref_pos) {
                            ...
                            bdsg::HashGraph graph;
                            algorithms::extract_context(*xindex, graph, xindex->get_handle(id(pos), is_rev(pos)),
                                                        offset(pos),
                                                        band.sequence().size() * extend_fwd, true, false);
                            algorithms::extract_context(*xindex, graph, xindex->get_handle(id(pos), is_rev(pos)),
                                                        offset(pos),
                                                        band.sequence().size() * extend_rev, false, true);
                            auto proposed_band = align_maybe_flip(band, graph, is_rev(pos), true, false,
                                                                  xdrop_alignment);
                            if (proposed_band.score() > max_score) {
                                band = proposed_band;
                                max_score = band.score();
                            }
                        }
                        // TODO
                    ...
                    if (band.has_path()) {
                        ...
                        if (from_length >= min_mem_length
                            ...
                            // strip back a little
                            if (to_strip.size() > k + 1 && to_strip[k + 1].first &&
                                band.sequence().size() > to_strip[k + 1].first) {
                    ...
                    if (band.path().mapping_size() == 0) {
                        ...
                        set <pos_t> next_pos;
                        for (auto &pos : band_ref_pos) {
                            for (auto &next : algorithms::jump_along_closest_path(xindex, pos, band.sequence().size(),
                                                                                  band.sequence().size())) {
...
AlignmentChainModel::AlignmentChainModel(
        ...
        Mapper * mapper,
const function<double(const Alignment &, const Alignment &, const unordered_map <path_handle_t, vector<
        pair < size_t, bool>>>&, const unordered_map <path_handle_t, vector<
        pair < size_t, bool>> >&, int64_t)>& transition_weight,
...
score = 0;
v.
positions = algorithms::alignment_path_offsets(*mapper->xindex, aln, true, false);
v.positions[handlegraph::as_path_handle(0)].
push_back(make_pair(mapper->approx_alignment_position(aln), false)
);
model.
push_back(v);
// mapper->counter[1]++;
...
for (
vector<AlignmentChainModelVertex>::iterator v = model.begin();
v != model.
...
v->next_cost.
push_back(make_pair(&*u, weight)
);
u->prev_cost.
push_back(make_pair(&*v, weight)
);
}
// mapper->counter[2]++;
} else {
...
FragmentLengthDistribution::FragmentLengthDistribution(size_t
maximum_sample_size,
size_t reestimation_frequency,
double robust_estimation_fraction
) :
